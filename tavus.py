...
from livekit.agents import JobContext, WorkerOptions, cli, WorkerPermissions, RoomOutputOptions, WorkerType
...
class UserData:
    ...
    def add_flash_card(self, question: str, answer: str) -> FlashCard:
        ...
        card = FlashCard(id=str(uuid.uuid4()),
                         question=question,
                         answer=answer)
    ...
    def add_quiz(self, questions: List[QuizQuestionDict]) -> Quiz:
        ...
        for q in questions:
            ...
            for a in q["answers"]:
                answers.append(
                    QuizAnswer(id=str(uuid.uuid4()),
                               text=a["text"],
                               is_correct=a["is_correct"]))
            quiz_questions.append(
                QuizQuestion(id=str(uuid.uuid4()),
                             text=q["text"],
                             answers=answers))
    ...
    def check_quiz_answers(self, quiz_id: str,
                           user_answers: dict) -> List[tuple]:
...
class AvatarAgent(Agent):
    ...
    @function_tool
    async def create_flash_card(self, context: RunContext[UserData],
    ...
    @function_tool
    async def flip_flash_card(self, context: RunContext[UserData],
                              card_id: str):
    ...
    @function_tool
    async def create_quiz(self, context: RunContext[UserData],
    ...
    @function_tool
    async def toggle_camera(self, context: RunContext[UserData],
                            enabled: bool):
        """Toggle the camera feed on/off."""
        self.camera_enabled = enabled
        if enabled:
            # Initialize camera stream
            self.camera_stream = await context.room.create_local_video_track()
            await context.room.publish_track(self.camera_stream)
        else:
            # Stop camera stream
            if self.camera_stream:
                await self.camera_stream.stop()
                self.camera_stream = None
        return {"status": "success", "camera_enabled": enabled}

    async def on_enter(self):
        """Handle room entry."""
        await super().on_enter()
        # Initialize camera in disabled state
        self.camera_enabled = False
        self.camera_stream = None
...
async def entrypoint(ctx: JobContext):
    ...
    async def handle_flip_flash_card(rpc_data):
        try:
            ...
            logger.error(
                f"JSON parsing error for payload '{rpc_data.payload}': {e}")
    ...
    async def handle_submit_quiz(rpc_data):
        try:
            ...
            correct_count = sum(1 for _, _, _, is_correct in quiz_results
                                if is_correct)
            ...
            for question, selected_answer, correct_answer, is_correct in quiz_results:
                if is_correct:
                    ...
                    card = userdata.add_flash_card(question.text,
                                                   correct_answer.text)
                    participant = next(
    ...
    ctx.room.local_participant.register_rpc_method("agent.flipFlashCard",
                                                   handle_flip_flash_card)

    ctx.room.local_participant.register_rpc_method("agent.submitQuiz",
                                                   handle_submit_quiz)
